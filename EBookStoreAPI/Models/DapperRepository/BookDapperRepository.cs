...
namespace EBookStoreAPI.Models.DapperRepository
{
    public class BookDapperRepository
    {
        ...
        {
            string sql = $@"SELECT top 5 [Books].Id AS Id,Books.[Name],BookImages.[Image] AS BookImage,SUM(OrderItems.Qty) as Qty
                            from [dbo].[OrderItems]
                            LEFT JOIN Books on [Books].Id=OrderItems.BookId
                            LEFT JOIN BookImages on BookImages.BookId=OrderItems.BookId
                            LEFT JOIN [Orders] on Orders.Id=OrderItems.OrderId
                            where(1=1) 
                            and Orders.OrderStatusId='2'
                            GROUP BY Books.[Name],BookImages.[Image],[Books].Id
                            Order By Qty desc";
